<form method="POST" action="{{ route('availability.update') }}" class="rounded-lg border border-gray-200 mx-2 w-fit" id="availabilityForm">
    ...
    <div class="flex flex-col sm:flex-row w-full">
        <label for="available" class="cursor-pointer flex-1">
            <input type="radio" name="availability" id="available" class="peer sr-only" value="aanwezig" {{ Auth::user()->device->status === 'aanwezig' && Auth::user()->device->updated_at->isToday() ? 'checked' : '' }} />
            <span class="relative inline-flex h-9 items-center space-x-2 py-2 pl-7 pr-3 text-sm peer-checked:bg-green-200 rounded-md w-60 sm:w-full">
        ...
        <label for="busy" class="cursor-pointer flex-1">
            <input type="radio" name="availability" id="busy" class="peer sr-only" value="bezet" {{ Auth::user()->device->status === 'bezet' && Auth::user()->device->updated_at->isToday() ? 'checked' : '' }} />
        ...
        <label for="away" class="cursor-pointer flex-1">
            <input type="radio" name="availability" id="away" class="peer sr-only" value="afwezig" {{ Auth::user()->device->status === 'afwezig' || !Auth::user()->device->updated_at->isToday() ? 'checked' : '' }} />
